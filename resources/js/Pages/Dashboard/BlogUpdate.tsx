...
export default function BlogUpdate({category, blog, auth} : PageProps<{category : BlogCategory[], blog: Blog}>) {
    const ref = useRef<TinyMCEEditor | null>(null);
    const [image, setImage] = useState<File|null>(null);
    const [imagePrev, setImagePrev] = useState<string|null>("/storage/" + blog.thumbnail);
    const [title, setTitle] = useState<string>(blog.title);
    const [categories, setCategories] = useState<string>(blog.category);
    ...
    const log = async () => {
        ...
        if (!ref.current || !title || !categories || !tags) return;
            router.post(route("dashboard.blog.update.post", blog.id), {
                title,
                tag : tags,
                category : categories,
                content,
                thumbnail : image ?? imagePrev
            });
      };
           ...
           <div className="grid grid-cols-3">
                <div className="p-5">
                    ...
                    <div className="grid w-full max-w-sm items-center gap-1.5 mb-3">
                        ...
                        <Input type="text" onChange={e => setTitle(e.target.value)} value={title} id="title" placeholder="title" />
                    </div>
                    <div className="grid w-full max-w-sm items-center gap-1.5 mb-3">
                        <Label htmlFor="category">Kategori</Label>
                        <Select onValueChange={e => setCategories(e)} defaultValue={categories}>
                    ...
                    <div className="grid w-full max-w-sm items-center gap-1.5 mb-3">
                        ...
                        <Input type="text" value={tags} onChange={e => setTags(e.target.value)} id="tags" placeholder="tags" />
                    </div>
                ...
                />
                    ...
                    <div className="mt-5 flex gap-5">
                        <Button onClick={log}>Perbarui Data</Button>
                        <Button variant={"destructive"}>Hapus Blog</Button>
                    </div>
                </div>
           </div>
        </PanelLayout>
